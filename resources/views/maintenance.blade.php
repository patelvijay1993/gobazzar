...
body{min-height:100vh;background:#0d1117;font-family:'Segoe UI',system-ui,sans-serif;display:flex;align-items:center;justify-content:center;overflow:hidden}

/* Grid background */
.grid-bg{position:fixed;inset:0;background-image:linear-gradient(rgba(26,58,143,.08) 1px,transparent 1px),linear-gradient(90deg,rgba(26,58,143,.08) 1px,transparent 1px);background-size:50px 50px;z-index:0}
.glow{position:fixed;top:50%;left:50%;transform:translate(-50%,-50%);width:800px;height:800px;background:radial-gradient(circle,rgba(26,58,143,.15) 0%,transparent 70%);z-index:0;pointer-events:none}

.container{position:relative;z-index:1;text-align:center;padding:40px 20px;max-width:600px}

/* Animated gear */
.gear-wrap{margin-bottom:40px;position:relative;display:inline-block}
.gear{font-size:80px;display:inline-block;animation:spin 4s linear infinite;filter:drop-shadow(0 0 20px rgba(240,165,0,.4))}
.gear2{font-size:48px;display:inline-block;animation:spin-rev 3s linear infinite;position:absolute;bottom:-8px;right:-16px;filter:drop-shadow(0 0 12px rgba(26,58,143,.5))}
...
.badge{display:inline-block;background:rgba(240,165,0,.1);border:1px solid rgba(240,165,0,.25);color:#f0a500;font-size:11px;font-weight:700;letter-spacing:2px;text-transform:uppercase;padding:6px 16px;border-radius:100px;margin-bottom:20px}

h1{font-size:clamp(28px,5vw,48px);font-weight:900;color:#fff;margin-bottom:12px;line-height:1.2}
.subtitle{font-size:16px;color:rgba(255,255,255,.5);margin-bottom:40px;line-height:1.7}

/* Status card */
.status-card{background:rgba(255,255,255,.03);border:1px solid rgba(255,255,255,.08);border-radius:20px;padding:28px;margin-bottom:36px}
...
/* Progress bar */
.maint-progress{margin-bottom:36px}
...
<div class="container">
  ...
  <p class="subtitle">We're performing scheduled maintenance to improve your experience.<br>Thank you for your patience — it won't take long!</p>
