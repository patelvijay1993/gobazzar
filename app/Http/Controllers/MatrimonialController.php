...
class MatrimonialController extends Controller
{
    public function index(Request $request)
    {
        $profiles = Matrimonial::where('status', 'active')
            ...
            ->when($request->religion, fn ($q) => $q->where('religion', 'like', '%' . addcslashes($request->religion, '%_\\') . '%'))
            ...
            ->when($request->search,   fn ($q) => $q->where(fn ($q2) => $q2
                ...
                ->orWhere('occupation', 'like', '%' . addcslashes($request->search, '%_\\') . '%')
                ->orWhere('religion',   'like', '%' . addcslashes($request->search, '%_\\') . '%')))
